...
import { FileSystemDirectoryEntry, FileSystemEntry, FileSystemFileEntry } from './interfaces/dom.types';
import { NgxFileDropContentTemplateDirective } from './directive/ngx-templates.directive';
...
export class NgxFileDropComponent implements OnDestroy {
  ...
  @ViewChild('fileSelector', { static: true }) fileSelector!: ElementRef<HTMLInputElement>;


  public isDraggingOverDropZone = signal<boolean>(false);

  private readonly globalDraggingInProgress = signal<boolean>(false);
  ...
  constructor() {
    this.globalDragStartListener = this.renderer.listen('document', 'dragstart', (evt: Event) => {
      console.log(evt)
      this.globalDraggingInProgress.set(true);
    });
          ...
          if (!result.length) {
            ...
            if (entries.length === 0) {
              ...
              for (const element of entries) {
                this.zone.run(() => {
                  this.traverseFileTree(element, path + element.name);
                });
              }
            }
  ...
  private isDropzoneDisabled(): boolean {
    return (this.globalDraggingInProgress() || this._disabled());
  }
